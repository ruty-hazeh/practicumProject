...
export default function PlaylistPage() {
  ...
  const generateSmartPlaylist = async () => {
    if (!selectedPlaylist?.id || !moodText.trim()) return;
    if (!context || !context.user?.id) return setError('משתמש לא מחובר');

    try {
      await axios.post(`https://localhost:7208/api/playlists/smart`, {
        moodText,
        userId: context.user.id,
      });
      console.log('generateSmartPlaylistNew called, moodText:', moodText);
